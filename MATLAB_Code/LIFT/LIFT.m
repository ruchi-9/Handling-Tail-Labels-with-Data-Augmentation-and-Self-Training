...
for i=1:num_class
    ...
    if(k1==0)
        POS_C=[];
        [NEG_IDX,NEG_C]=kmeans(train_data,min(50,num_train),'EmptyAction','singleton','OnlinePhase','off','Display','off','MaxIter',200);
    else
        if(size(p_data,1)==1)
            POS_C=p_data;
        else
            [POS_IDX,POS_C]=kmeans(p_data,k1,'EmptyAction','singleton','OnlinePhase','off','Display','off','MaxIter',200);
        end
        
        if(size(n_data,1)==1)
            NEG_C=n_data;
        else
            [NEG_IDX,NEG_C]=kmeans(n_data,k2,'EmptyAction','singleton','OnlinePhase','off','Display','off','MaxIter',200);
        end
    end
...
for i=1:num_class
    ...
    Models{i,1}=svmtrain(full(training_label_vector),training_instance_matrix,str);
end
...
for i=1:num_class
    ...
    [predicted_label,accuracy,prob_estimates]=svmpredict(full(testing_label_vector),testing_instance_matrix,Models{i,1},'-b 1');
